...
class UserController extends BaseController {
    ...
    public function logout(){
        ...
        session_unset();
        session_destroy();
        if($this->broker){$this->broker->logout();}
    ...
    public function do_login(){
    	...
    	$logic = new UsersLogic();
    	$res = $logic->login($username,$password,$this->broker);        
    	if($res['status'] == 1){
            ...
            if(!empty($res['result']['username'])){
                try{
                    $this->broker->login($res['result']['username'],$password);
                }catch (\Exception $e) {
                    $res['status'] = -4;
                    $res['msg'] = '密码错误!(Broker)';
                    exit(json_encode($res));
    ...
    public function reg(){
        ...
        if(IS_POST){
            ...
            // $result= $this->broker->__call('checkusername',['username'=>$username]);
            // if($result['state'] =='success'){
            //     $res=$this->broker->__call('register',['username'=>$username,'password'=>$password]);
            // }
        ...
        //     if($result == 'NULL'){
        //    $result=$this->broker->register($username,$password);
        //    
    ...
    */
    ...
    public function password(){
        ...
        if(IS_POST){
            ...
            $confirm = I('post.confirm_password');
            $data = $userLogic->password($this->user_id,$old,$new,$confirm); // 获取用户信息
            if($data['status'] == -1){
                $this->error($data['msg']);
            }
            $result = $this->broker->__call('changepassword',['newpassword'=>$new,'username'=>$user['username'],'oldpassword'=>$old]);
