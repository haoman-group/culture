...
class UsersLogic extends RelationModel
{
     ...
     * 登陆
     */
    public function login($username,$password,$broker){
    	$result = array();
        ...
        if(!$user){
            ...
            if($user){
                ...
                $chkresult = $broker->__call('checkusername',['username'=>$username]);
                if($chkresult['state'] =='fail'){
                    $user = array();
                    $user['username'] = $username;
                    $user['nickname'] = $username;
                    $user['is_distribut'] = 1;
                    $user['reg_time'] = time();
                    $user['first_leader'] = cookie('first_leader'); // 推荐人id
                    $user['user_id'] = M('users')->add($user);
                    if($user['user_id']){
                        $result = array('status'=>1,'msg'=>'登陆成功','result'=>$user);
                    }else{
                        $result = array('status'=>-1,'msg'=>'账号不存在!','data'=>M('users')->getError());    
                    }
                }else{
                    $result = array('status'=>-1,'msg'=>'账号不存在!','data'=>$chkresult);
                }
            }
...
//        	}
    ...
    public function password($user_id,$old_password,$new_password,$confirm_password,$is_update=true){
        ...
        if($is_update && ($user['password'] != '' && encrypt($old_password) != $user['password']))
            return array('status'=>-1,'msg'=>'旧密码验证失败','result'=>'');
