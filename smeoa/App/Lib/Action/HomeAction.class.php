...
class HomeAction extends CommonAction {
	...
	public function index() {
		...
		// var_dump($menu);
		$area=M('Area')->where(array('id'=>array(array('GT',25000000),array('LT',26000000))))->field('id,pid,name')-> order('id desc')->select();

		// var_dump($area);
		foreach($area as $key=>$value){
			$area[$key]['is_del']=0;
		}
		$area_tree = list_to_tree($area,25000000);
		$tree = list_to_tree($menu);
		
		$this -> assign('menu', popup_tree_menu($tree));
		$this -> assign('area',popup_tree_menu($area_tree));
