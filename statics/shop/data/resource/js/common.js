...
function showTips( tips, height, time ){
    ...
    $( 'div.tipsClass' ).css({
        ...
        'opacity' : 'http://www.sdcii.com:81/data/resource/js/0.90',
        'z-index' : '9999'
    }).show();
...
function collect_store(fav_id,jstype,jsobj){
    $.get('http://www.sdcii.com:81/data/resource/js/index.php?act=index&op=login', function(result){
        if(result=='0'){
            login_dialog();
        }else{
            var url = 'http://www.sdcii.com:81/data/resource/js/index.php?act=member_favorites&op=favoritesstore';
...
function collect_goods(fav_id,jstype,jsobj){
    $.get('http://www.sdcii.com:81/data/resource/js/index.php?act=index&op=login', function(result){
        if(result=='0'){
            ...
            var url = 'http://www.sdcii.com:81/data/resource/js/index.php?act=member_favorites&op=favoritesgoods';
            $.getJSON(url, {'fid':fav_id}, function(data){
                if (data.done)
            {
                showDialog(data.msg, 'succ','','','','','','','','',2);
        ...
        if(result.state){
            ...
            if(result.quantity == 0){
                ...
                if ($("#rtoolbar_cartlist > ul").children().size() != result.quantity) {
                    $("#rtoolbar_cartlist").load("http://www.sdcii.com:81/data/resource/js/index.php?act=cart&op=ajax_load&type=html");
                    return
                }
            }
        }else{
...
(function($) {
    $.show_nc_login = function(options) {
        var settings = $.extend({}, {action:'../../../psapp/member/logon.do.htm'/*tpa=http://www.sdcii.com:81/index.php?act=login&op=login&inajax=1*/, nchash:'', formhash:'' ,anchor:''}, options);
        ...
        login_dialog_html.append('<form class="bg" method="post" id="ajax_login" action="'+APP_SITE_URL+settings.action+'"></form>').find('form')
        	...
        	.append('<ul><li>›&nbsp;如果您没有登录帐号，请先<a class="register" href="http://www.sdcii.com:81/data/resource/js/'+SHOP_SITE_URL+'/index.php?act=login&op=register">注册会员</a>然后登录</li><li>›&nbsp;如果您<a class="forget" href="http://www.sdcii.com:81/data/resource/js/'+SHOP_SITE_URL+'/index.php?act=login&op=forget_password">忘记密码</a>？，申请找回密码</li></ul>')
 ...
 * <link href="--php echo RESOURCE_SITE_URL;---js-qtip-jquery.qtip.min.css"/*tpa=http://www.sdcii.com:81/data/resource/js/<?php echo RESOURCE_SITE_URL;?>/js/qtip/jquery.qtip.min.css*/ rel="stylesheet" type="text/css">
...
(function($){
	$.fn.membershipCard = function(options){
		                ...
		                success: function(data) {
		                	if(data){
		                		...
		                		$('<dt class="member-id"></dt>').append('<i class="sex'+data.sex+'"></i>')
		                			.append('<a href="http://www.sdcii.com:81/data/resource/js/'+SHOP_SITE_URL+'/index.php?act=member_snshome&mid='+data.id+'" target="_blank">'+data.name+'</a>'+(data.truename != ''?'('+data.truename+')':''))
		                			.appendTo(_dl);
		                		// avatar
		                		$('<dd class="avatar"><a href="http://www.sdcii.com:81/data/resource/js/'+SHOP_SITE_URL+'/index.php?act=member_snshome&mid='+data.id+'" target="_blank"><img src="'+data.avatar+'" /></a><dd>')
		                			.appendTo(_dl);
		                		// info
		                		var _info = '';
		                		...
		                		if(data.follow != 2){
		                			...
		                			$('<div class="send-msg"> <a href="http://www.sdcii.com:81/data/resource/js/'+SHOP_SITE_URL+'/index.php?act=member_message&op=sendmsg&member_id='+data.id+'" target="_blank"><i></i>站内信</a> </div>').appendTo(_bottom);
		                		}

		                		var _content = $('<div class="member-card"></div>').append(_dl).append(_bottom);
			                    this.set('http://www.sdcii.com:81/data/resource/js/content.text', ' ');this.set('http://www.sdcii.com:81/data/resource/js/content.text', _content);
		                	}
		                }
	                }
	            },
		...
		function circle_function(d){
			...
			$.each(d,function(i, n){
				rs.append('<div class="rank-div" title="'+n.circle_name+'圈等级'+n.cm_level+'，经验值'+n.cm_exp+'"><a href="http://www.sdcii.com:81/data/resource/js/'+CIRCLE_SITE_URL+'/index.php?act=group&c_id='+n.circle_id+'" target="_blank">'+n.circle_name+'</a><i></i><em class="rank-em rank-'+n.cm_level+'">'+n.cm_level+'</em></div>');
			});
			return rs;
		}
    ...
    $.fn.nc_region = function(options) {
        ...
        function loadAreaArray(callback) {
            if(typeof nc_a === 'undefined') {
                ...
                area_scripts_src = $("script[src*='jquery.js'/*tpa=http://www.sdcii.com:81/data/resource/js/jquery.js*/]").attr("src");
                area_scripts_src = area_scripts_src.replace('jquery.js'/*tpa=http://www.sdcii.com:81/data/resource/js/jquery.js*/, 'area_array.js'/*tpa=http://www.sdcii.com:81/data/resource/js/area_array.js*/);
                $.ajax({
...
function addcart(goods_id,quantity,callbackfunc) {
    var url = 'http://www.sdcii.com:81/data/resource/js/index.php?act=cart&op=add';
