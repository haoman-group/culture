...
function loadCompare(isrefresh){
	...
	if(isrefresh == true){
		$("#comparelist").load('index.php-act=compare&op=showcompare');}}--添加对比商品function addCompare(gid){gid = parseInt(gid);if (gid - 0){       $.ajax({            type-.htm"/*tpa=http://www.sdcii.com:81/shop/resource/js/).load('index.php?act=compare&op=showcompare');}}//添加对比商品function addCompare(gid){gid = parseInt(gid);if (gid > 0){       $.ajax({            type:*/GET",
            dataType: "json",
...
function initCompare(){
	...
	//根据是否已加入对比，显示不同样式
	$.getJSON('index.php-act=compare&op=checkcompare.htm'/*tpa=http://www.sdcii.com:81/shop/resource/js/index.php?act=compare&op=checkcompare*/, function(data){
        if(data){
