...
/**
 ...
 authkey 参数密钥，验证args
 **/
function videoupload(uploadid, name, textareaid, funcName, args, module, catid, authkey) {
    var args = args ? '&args=' + args : '';
    var setting = '&module=' + module +'&catid=' + catid + '&authkey=' + authkey;
    var status = false;
    //检查是否有上传权限
    ...
    if (status) {
        Wind.use("artDialog", "iframeTools", function () {
            art.dialog.open(GV.DIMAUB + 'index.php?a=videoupload&m=Attachments&g=Attachment' + args + setting, {
                title: name,
                id: uploadid,
                width: '650px',
                height: '420px',
...
function audioupload(uploadid, name, textareaid, funcName, args, module, catid, authkey) {
    var args = args ? '&args=' + args : '';
    var setting = '&module=' + module +'&catid=' + catid + '&authkey=' + authkey;
    var status = false;
    //检查是否有上传权限
    ...
    if (status) {
        Wind.use("artDialog", "iframeTools", function () {
            art.dialog.open(GV.DIMAUB + 'index.php?a=audioupload&m=Attachments&g=Attachment' + args + setting, {
                title: name,
                id: uploadid,
                width: '650px',
                height: '420px',
...
function audioplay(audio_path, audio_title) {
    var status = false;
    //检查文件是否存在
    if (window.ActiveXObject) {
        var xmlhttp = ActiveXObject("Microsoft.XMLDOM");
    } else {
        var xmlhttp = new XMLHttpRequest();
    }
    xmlhttp.open("GET",audio_path,false);
    xmlhttp.send();
    ...
    var args = "&audio_path=" + audio_path + "&audio_title" + audio_title;
    if (status) {
        Wind.use("artDialog", "iframeTools", function () {
            art.dialog.open(GV.DIMAUB + 'index.php?a=audioplay&m=Attachments&g=Attachment' + args, {
                title: audio_title,
                // width: '650px',
                // height: '650px',
                lock: true,
