var register = {
    init: function () {
        ...
        $rmobile.focus(function(){
            $(this).addClass('input_focus');
            $mmobile.html('<div class="err_message"><span class="icon">请输入手机号。</span></div>');
        }).blur(function(){
            $(this).removeClass('input_focus');
            $(this).triggerHandler('keyup');
        }).keyup(function(){
            ...
            if ($mobile == '' || $mobile.length != 11) {
                $mmobile.html('<div class="err_message error"><span class="icon">请输入手机号。</span></div>');
                return false;
            } else if(!myreg.test($mobile)){
                $mmobile.html('<div class="err_message error"><span class="icon">请输入正确的手机号。</span></div>');
                return false;
            }   
            else {
                $mmobile.html('<div class="err_message right"><span class="rightIcon"></span></div>');
            }
            $.ajax({
                type: "POST",
                global: false, // 禁用全局Ajax事件.
                url: _config['domainSite'] + "index.php?g=Member&m=Public&a=checkMobile",
                data: {
                    mobile: $rmobile.val()
                },
                dataType: "json",
                success: function (data) {
					if(data.status){
						...
						$mmobile.html('<div class="err_message error"><span class="icon">'+data.info+'</span></div>');
						return false;
					}
                },
                error: function () {
                    ...
                    success: function (data) {
						if(data.status){
                            ...
                            $.tipMessage('帐号注册成功！', 0, 2000,0,function(){
                                libs.redirect(_config['domainSite']+'index.php?g=Member&a=regavatar&type='+type);
                                //alert(_config['domainSite']+'index.php?g=Member&a=regavatar&type'+type);
