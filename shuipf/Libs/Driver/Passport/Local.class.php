...
class Local extends Passport {
    ...
    public function loginLocal($identifier, $password = null, $is_remember_me = 604800) {
       ...
       //var_dump($password);exit;
        ...
        if (!empty($password)) {
            ...
            //对明文密码进行加密
            $password_en = D("Member/Member")->encryption($identifier, $password, $encrypt);
            if ($password_en != $userinfo['password']) {
                $this->error = '密码错误！';
                return false;
            }
        }
        ...
        if ($this->registerLogin($userinfo, $is_remember_me)) {
            ...
            if($sso_config['SSO_SERVER_OPEN']=="true"){
               MyBroker::login($identifier, $password);
            }
    ...
    /**
    */
      public function loginLibrary($identifier, $password = null, $is_remember_me = 604800) {
        $map = array();
        if (is_int($identifier)) {
            $map['userid'] = $identifier;
            //echo $map['userid'];
