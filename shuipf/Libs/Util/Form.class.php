...
class Form {
     ...
     */
    public static function editor($textareaid = 'content', $toolbar = 'basic', $module = '', $readonly=false, $catid = '', $allowupload = 0, $allowbrowser = 1, $alowuploadexts = '', $allowuploadnum = '10', $height = 200, $disabled_page = 0) {
        $str = "";
    ...
    public static function multi_images($name, $id = '', $value = '', $moudle = '', $catid = '', $size = 50, $class = 'input', $ext = '', $alowexts = '', $thumb_setting = array(), $watermark_setting = 0) {
        ...
        //return $str . " <input type=\"button\" class=\"button\" onclick=\"javascript:flashupload('{$id}_images', '附件上传','{$id}',house_images,'1,{$alowexts},1,{$thumb_ext},{$watermark_setting}','{$moudle}','{$catid}','{$authkey}')\"/ value=\"上传图片\"> <div id=\"$id\"></div> ";
        $str .= "<input class='button' type='button' onclick=\"flashupload('{$id}_images', '附件上传','{$id}',change_multifile_industry,'8,{$alowexts},1,{$thumb_ext},{$watermark_setting}','{$moudle}','{$catid}','{$authkey}')\" style=\"background: url('/statics/images/icon/tianjia2.jpg') no-repeat -1px 0;vertical-align: top\" />";
        //$str .= "<div id='{$id}'>";
        $str .= "<ul class='tpgl' id='{$id}'>";
        foreach ($value as $k => $v) {
            $str .= "<li id='picture_{$k}' class='bianji'><input type='hidden' name='{$id}_url[]' value='{$v}'><img src='{$v}' class='img'><p><a href=\"javascript:remove_div('picture_{$k}')\" >删除</a></p></li>";
        }
        $str .="</ul>";
        //$str .= "<div class='bianji1'><p><a href=\"javascript:flashupload('{$id}_images', '附件上传','{$id}',house_images,'8,{$alowexts},1,{$thumb_ext},{$watermark_setting}','{$moudle}','{$catid}','{$authkey}')\">添加图片</a></p></div>";
        return $str;
    }
}