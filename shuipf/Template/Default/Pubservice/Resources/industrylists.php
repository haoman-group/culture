				...
				<div class="resocatetit">
					...
					<volist  name="data['category']" id="vo">
							<if condition="$vo['is_parent'] eq '1'">
								<a class="<if condition='$key eq 0'>on</if>" href="{:U('industrylists',array('cid'=>$vo['cid']))}">{$vo.name}</a>
							<else/>
								<a class="<if condition='$key eq 0'>on</if>" href="{:U('industryparent',array('cid'=>$vo['cid']))}">{$vo.name}</a>
							</if>
						</if>
					</volist>
