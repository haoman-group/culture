                        ...
                        <content action = "lists" catid = "$catid" order = "id DESC" num= "15" page = "$page">
                            ...
                            <ul class="local">
                                <volist name="data" id="vo">
                                    <li><a href="{:U('Industry/Index/shows',array('catid'=>$catid,'id'=>$vo['id']))}"><em>●</em>{$vo.title|str_cut=###,40}</a><span>{$vo.inputtime|date="Y-m-d",###}</span></li>
                                </volist>
                            </ul>
                    ...
                    <div class="newsbox">
                        ...
                        <ul class="rtrankul">
                            <content action ="hits" catid="$catid" order="views DESC" num="7">
                                <volist name = "data" id = "vo">
                                    <if condition = "($i lt 4)">
                                        <li class="on"><span>{$i}</span><a href="{:U('Industry/Index/shows',array('catid'=>$catid,'id'=>$vo['id']))}" style="color:#696b76; ">{$vo.title|str_cut=###,23,''}</a></li>
                                        <else />
                                        <li><span style="background: #C9C9C9">{$i}</span><a href="{:U('Industry/Index/shows',array('catid'=>$catid,'id'=>$vo['id']))}" style="color:#696b76;" >{$vo.title|str_cut=###,23,''}</a></li>
                                    </if>
                                </volist>
                            </content>
