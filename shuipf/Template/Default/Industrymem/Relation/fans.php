...
<title>我的粉丝 - {$Config.sitename}</title>
      ...
      <div class="uMain_content">
        <div class="main_nav">
          <ul>
            ...
            <li class="fans"> <a class="on" href="{:U('Relation/fans')}">我的粉丝</a> </li>
        ...
        <div class="minHeight500">
          <div class="relation_userlist"> 
            ...
            <div class="nothing">暂时还没有粉丝啊！</div>
            </if>
            <ul>
             <volist name="friends" id="vo">
              <li>
                <div class="avatar"> <a target="_blank" href="{:UM('Home/index',array('userid'=>$vo['userid']))}" class="user_card" uid="{$vo.userid}"> <img class="avatar-80" src="{:U('Api/Avatar/index',array('uid'=>$vo['userid'],'size'=>90))}" onerror="this.src='{$model_extresdir}images/noavatar.jpg'"/> </a> </div>
                <ul class="info">
                  <li> <a  class="<if condition=" $vo['isonline'] ">online_icon</if> user_card" target="_blank" href="{:UM('Home/index',array('userid'=>$vo['userid']))}" uid="{$vo.userid}">{$userList[$vo['userid']]['username']}</a> </li>
                  <li>粉丝<strong>{$userList[$vo['userid']]['fans']}</strong>人</li>
