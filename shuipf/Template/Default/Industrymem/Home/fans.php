...
<meta name="description" content="查看{$userinfo.username}的粉丝团，了解{$userinfo.username}喜欢的资讯信息，与他一起交流，分享关注{$userinfo.username}的粉丝团信息。" />
<meta name="keywords" content="{$userinfo.username}的粉丝团" />
<title>{$userinfo.username}的粉丝团 - {$Config.sitename}</title>
      ...
      <div class="stageBox">
        ...
        <div class="stageBoxCenter min_space_height">
          <div class="spaceMTitle2">
            <p>{$my}的关系</p>
            <ul>
              <li><a href="{:UM('Home/following',array('userid'=>$userinfo['userid']))}">{$my}关注的人</a><b></b></li>
              <li class="current"><a href="{:UM('Home/fans',array('userid'=>$userinfo['userid']))}">{$my}的粉丝团</a><b></b></li>
              <if condition="$uid && $ismyhome ">
              <li class="management"><a href="{:U('Relation/fans')}">管理</a></li>
              </if>
            </ul>
          </div>
          <div class="title_per"></div>
          <div class="fansList"><!--有内容-->
            <if condition=" empty($fans) ">
              <div class="nothing_fans">还没有任何粉丝。</div>
            </if>
            <ul>
              <volist name="fans" id="vo">
                <li>
                  <div class="avatar"><a href="{:UM('Home/index',array('userid'=>$vo['userid']))}" class="user_card" uid="{$vo['userid']}"><img class="avatar-80" width="80" height="80" src="{:U('Api/Avatar/index',array('uid'=>$vo['userid'],'size'=>90))}" onerror="this.src='{$model_extresdir}images/noavatar.jpg'"/></a></div>
                  <ul class="info">
                    <li><a class="<if condition=" $vo['isonline'] ">online_icon</if> user_card" uid="{$vo['userid']}"  title="ahu" href="{:UM('Home/index',array('userid'=>$vo['userid']))}">{$vo.username}</a></li>
                    <li>关注<strong>{$vo.attention}</strong>人 、 粉丝<strong>{$vo.fans}</strong>人</li>
