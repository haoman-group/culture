...
class IndexController extends Base {
    ...
    public function index() {
        // $this->redirect('Pubservice/index/index');exit;
        ...
        //获取地区数据
        //1.获取地区列表
        $citylist = M('Area')->where(['pid'=>"25000000"])->order('id desc')->cache(true)->getField('id,name');
        $area_id = I('get.id',null)?I('get.id',null):cookie('culture_area_id');
        $cityname = null;
        $slideData = null;
        if(empty($area_id)){
            $cityname = '全省';
            $slideData = '';
            cookie('culture_area_id',null);
        }else{
            $cityname = M('Area')->where(['id'=>$area_id])->getField('name');
            //2.获取地区轮播数据
            $slideData = D('Admin/AreaData')->where(['area_id'=>$area_id])->getField('index_slide');
            $slideData = unserialize($slideData);
            cookie('culture_area_id',$area_id);
        }
    ...
    public function shows() {
        ...
        $contents=M('NewsData')->where(array('id'=>$id))->getField("content");
        ...
        //M('News')->where(array('id'=>$id))->setInc('views',1);
