...
class MemberBase extends Base {
    ...
    final public function check_member() {
        ...
        //特定模块无需登陆验证
        if (MODULE_NAME == 'Industrymem' && in_array(CONTROLLER_NAME, array('Public', 'Home'))) {
            //Public,Home不用验证登陆
        ...
        //特定方法无需登陆验证
        if (MODULE_NAME == 'Industrymem' && CONTROLLER_NAME == 'Index' && in_array(ACTION_NAME, array('login', 'register', 'logout', 'lostpassword', 'resetpassword', 'verifyemail'))) {
            //该类方法不需要验证是否登陆
            return true;
        }
        if ($this->userid) {
            ...
            if ($this->userinfo['groupid'] == 1) {
                ...
                $this->error("您的会员组为禁止访问！", config('Config.siteurl'));
            } else if ($this->userinfo['groupid'] == 7) {//邮箱认证
                service("Passport")->logoutLocal();
                $this->error("您还没有进行邮箱认证！", config('Config.siteurl'));
            }
            //锁定用户
            if ($this->userinfo['islock'] == 1) {
                service("Passport")->logoutLocal();
                $this->error("您的帐号已经被锁定！", config('Config.siteurl'));
            }
            return true;
        } else {
            //对待授权登陆的，进行特别处理把！
            if ($this->isConnectLogin) {
                ...
                if (MODULE_NAME == 'Member' && in_array(CONTROLLER_NAME, array('Index', 'Feed')) && in_array(ACTION_NAME, array('index', 'home', 'logout', 'fetchfeed'))) {
                    ...
                    $this->error('请完善资料后在继续操作！<a href="' . U("Industrymem/Public/connectregister") . '">Go...马上去完善资料？</a>');
                }
            }
            service("Passport")->logoutLocal();
            ...
            $this->redirect('Industrymem/Public/login');
        }
    }
