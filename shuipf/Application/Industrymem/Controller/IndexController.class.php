...
class IndexController extends MemberBase {
    ...
    public function home() {
        $HomeRun = D('Member/HomeRun');
        //取得用户的访客信息
        $visitors = $HomeRun->getUserVisitorsList($this->userid, 9);
        //我关注的人总数
        $friendsCount = $HomeRun->friendsCount($this->userid);
        //我关注的人
        $friends = $HomeRun->getFriendUserInfo($this->userid, 9);
    ...
    public function verifyemail() {
        ...
        if (empty($userinfo)) {
            $this->error('该帐号不存在，无法进行邮箱验证！', U('Industrymem/Index/index'));
        }
        if ($userinfo['checked']) {
            $this->success('该帐号已经验证通过！', U('Industrymem/Index/index'));
        }
        $data = array();
        $data['checked'] = 1;
        ...
        if (false !== $this->memberDb->where(array('userid' => $userinfo['userid']))->save($data)) {
            $this->success('邮箱验证完成！', U('Industrymem/Index/index'));
        } else {
            $this->error('邮箱验证失败！', U('Industrymem/Index/index'));
        }
    }
