...
use Think\Controller;
	...
	class BusinessController extends IndustryBaseController{
		...
		public function do_business_alliance(){
				if(IS_POST){
		            $data= I("post.");
					$address=M('area')->where(array("id"=>$_POST['area'][1]))->find();
				...
				if(empty($_POST['survey'])){
		            ...
		            if( !($res = $credit -> join() ) ){
		                ...
		                $this->success('新增成功',U('Industry/Index/lists',array('catid'=>21)));
		            }
		        }			
		}
