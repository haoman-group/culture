...
class CuBaseController extends Base
{
...
//        区域统计信息
       ...
       $massart_totle = D('Admin/Massart')->count();
       $this->assign('massart_totle',$massart_totle);
    }
    ...
    //lists页面面包屑导航
    public function _Get_breadcrumb($cid = '')
    {
        if (empty($cid) || $cid == '') {
            return false;
        }
        ...
        if ($category['parent_cid'] === '0') {
            $current_cate = "<a href='/index.php?g=Cudatabase'>" . $category['name'] . "</a>";
        } else {
            $current_cate = $category['name'];
        }
        $this->breadcrumb = $current_cate . ">" . $this->breadcrumb;
        if ($category['parent_cid'] != '0') {
            $this->_Get_breadcrumb($category['parent_cid']);
        }
