...
<form action="{:U('Admin/Library/addFund')}" method="post" class="J_ajaxForm">
    <dl class="logdata">
        <dt>财政拨款总额</dt><dd><input type="text" name='fund_totle' value="{$data.fund_totle}" /><span>（万元）</span><Form function="select" parameter="option('fund_source'),$data['fund_source'],class='' name='fund_source',"/></dd>
        <dt>新增藏量购置费</dt><dd><input type="text" name='fund_new' value="{$data.fund_new}" /><span>（万元）</span></dd>
        <dt>免费开放本地经费</dt><dd><input type="text" name='fund_free' value="{$data.fund_free}" /><span>（万元）</span></dd>
        <dt>电子资源购置费</dt><dd><input type="text" name='fund_ele' value="{$data.fund_ele}" /><span>（万元）</span></dd>
    </dl>
