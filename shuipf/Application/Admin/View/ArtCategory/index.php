  ...
  <table width="100%" cellspacing="0">
        <thead>
          <tr>
            ...
            <td align="center" width="50">关联关系</td>
        ...
        <volist name="data" id="vo">
          <tr>
            ...
            <td align="center">{$vo.relation}</td>
            <td align="center"><?php echo date("Y-m-d h:i:s",$vo['addtime']);  ?></td>
            <td align="center">
            <?php
			$op = array();
			if($vo['is_parent'] == 1){
				$op[] = '<a href="'.U('ArtCategory/child',array('cid'=>$vo['cid'])).'">子菜单</a>';
			}
//			$op[] = '<a href="'.U('Property/index',array('cid'=>$vo['cid'],'path' => $path)).'">属性</a>';
		// 	if(\Libs\System\RBAC::authenticate('edit')){
				$op[] = '<a href="'.U('ArtCategory/edit',array('cid'=>$vo['cid'])).'">修改</a>';
		// 	}
		// 	if(\Libs\System\RBAC::authenticate('delete')){
				$op[] = '<a class="J_ajax_del" href="'.U('ArtCategory/delete',array('cid'=>$vo['cid'])).'">删除</a>';
		// 	}
    if($vo['is_parent'] == 1){
        $op[] = '<a class="" href="'.U('ArtCategory/add',array('cid'=>$vo['cid'])).'">添加子项</a>';
    }
			echo implode(" | ",$op);
			?>
            </td>
