...
class CityContentController extends AdminBase {
    private $model = NULL;
    protected $Page_Size=25;
    protected function _initialize() {
        parent::_initialize();
        $this->model = D('Admin/CityContent');
        $this->assign('maxPicNum',1);
        $this->assign('category_array',$this->model->getCateArray());
    }
    //首页列表
    ...
    public function add(){
        if(IS_POST){
            $data = I('post.',null);
            $data['operater']=User::getInstance()->getInfo()['id'];
            $data['area'] = User::getInstance()->getInfo()['area'];
            $data['content']=htmlspecialchars_decode($data['content']);
            $data['cover']=$data['images_url'][0];
