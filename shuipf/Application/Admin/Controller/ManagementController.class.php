...
class ManagementController extends AdminBase {
    ...
    public function manager() {
       ...
       if(!empty(I('get.username'))){
        ...
        $this->assign("Custom", D('Admin/Menu')->getMenu(22));
        $this->display();
    ...
    public function adminadd() {
        if (IS_POST) {
            ...
            if (D('Admin/User')->createManager($_POST)) {
                $this->success("添加管理员成功！", U('Admin/Management/manager'));
            } else {
                $error = D('Admin/User')->getError();
    ...
    public function checkusernamename(){
        $username = I('post.username');
        $check=D('User')->where(array('username'=>$username))->find();
        
        if(!empty($check) ){
           
             $this->ajaxReturn(array('status'=>0,'infor'=>'存在'));

        }else{
           
            $this->ajaxReturn(array('status'=>1,'infor'=>'不存在'));
        }
    }
